...
# as well as utils for eval/validation
...
class BlacklistLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        for b_idx in range(input_ids.shape[0]):
            ...
            blacklist_ids = torch.randperm(self.vocab_size, device=input_ids.device, generator=self.g_cuda)[
                            :bl_ct]  # ty Yuxin :]
...
def generate_completions(example: dict,
    ...
    with torch.no_grad():
        ...
        while (success is False) and (samples_taken < max_retries):
            bl_processor_list[0].set_signature()
            ...
            start_generation = time.time()
            outputs_w_bl = w_bl_partial(inputs.to(model.device), logits_processor=bl_processor_list)
            # w_bl_partial(logits_processor=bl_processor_list)
...
def compute_ppl_single(prefix_and_output_text=None,
    ...
    with torch.no_grad():
        tokd_prefix = \
            tokenize_and_truncate({"text": prefix_and_output_text}, completion_length=0,
                                  hf_model_name=oracle_model_name,
                                  tokenizer=oracle_tokenizer,
                                  model_max_seq_len=oracle_model.config.max_position_embeddings)["inputs"]
        tokd_inputs = tokd_prefix
        # if only want to score the "generation" part we need the suffix tokenization length
        tokd_suffix = tokenize_and_truncate({"text": output_text}, completion_length=0, hf_model_name=oracle_model_name,
                                            tokenizer=oracle_tokenizer,
                                            model_max_seq_len=oracle_model.config.max_position_embeddings)["inputs"]
...
def evaluate_generation_fluency(example: dict,
    ...
    # add metrics
    loss, ppl = compute_ppl_single(inputs_plus_baseline_output, baseline_output, oracle_model_name, oracle_model,
                                   oracle_tokenizer)
    example["baseline_loss"] = loss
    example["baseline_ppl"] = ppl
    loss, ppl = compute_ppl_single(inputs_plus_no_bl_output, no_bl_output, oracle_model_name, oracle_model,
                                   oracle_tokenizer)
    example["no_bl_loss"] = loss
    example["no_bl_ppl"] = ppl
    loss, ppl = compute_ppl_single(inputs_plus_w_bl_output, w_bl_output, oracle_model_name, oracle_model,
                                   oracle_tokenizer)
